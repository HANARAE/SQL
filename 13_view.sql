...
뷰는 기본 테이블로 유도된 가상 테이블이기 때문에 필요한 컬럼만 저장해 두면 관리가 용이해진다.
뷰는 가상테이블로 실제 데이터가 물리적으로 저장된 형태는 아님.
...
원본 테이블이 NOT NULL인 경우 VIEW에 INSERT가 불가능함.
VIEW에서 사용하는 컬럼이 가상 열인 경우에도 불가능함.
*/
