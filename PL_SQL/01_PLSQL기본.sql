...
- 일반적인 프로그래밍과는 차이가 있지만, 오라클 내부에서 적절한 처리를 위해 적용해 줄 수 있는 절차지향적 코드 작성 방식이다.
...
특이한 점은 SELECT절 아래에 INTO절을 사용해서 변수에 조회 결과를 할당할 수 있다.
*/

-- 해당 테이블과 같은 타입의 컬럼 변수를 선언하려면
-- 테이블명.컬럼명%TYPE을 사용함으로써 타입을 일일이 확인해야 하는 번거오룸을 방지할 수 있다.
DECLARE
    v_emp_name employees.first_name%TYPE; -- 사원명 변수 (문자열 변수는 길이제약 필요)
