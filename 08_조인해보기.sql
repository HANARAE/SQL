...
/*
# 조인이란?
- 서로 다른 테이블간에 설정된 관계가 결합하여 1개 이상의 테이블에서 데이터를 조회하기 위해 사용한다.
...
SELECT
    ...
    e.salary, e.job_id, d.department_id, d.department_name
FROM employees e
INNER JOIN departments d
ON e.department_id = d.department_id;  --ANSI 표준 조인 문법

/*
각각의 테이블에 독립적으로 존재하는 컬럼의 경우에는 테이블 이름을 생략해도 무방.
그러나 해석의 명확성을 위해 테이블 이름을 작성하여 소속을 표현해 주는 것이 바람직하다.
테이블 이름이 너무 길 시에는 ALIAS(별칭)를 작성하여 칭한다.
두 테이블 모두 가지고 있는 컬럼의 경우에는 반드시 명시해줘야 한다.
*/


-- 3개의 테이블을 이용한 내부 조인(INNER JOIN)
-- 내부조인: 두 테이블 모두에서 일치하는 값을 가진 행만 반환
SELECT
...
3. 위의 결과와 동일한 department_id를 가진 employees 테이블의 데이터를 찾아서 조인
...
(+)가 붙지 않은 테이블을 기준으로 하여 departments 테이블이 조인에 참여하라는 의미를 부여하기 위해
기호를 붙인다.
외부 조인을 사용했더라도, 이후에 내부 조인을 사용하면 내부조인을 우선적으로 인식한다.
*/

/*
-- 외부 조인 진행시 모든 조건에 (+)를 붙여야 외부 조인이 유지됨.
-- 일반 조건에도 (+)를 붙이지 않으면 외부 조인이 풀리는 현상이 발생.(데이터 누락 발생)
*/
SELECT
