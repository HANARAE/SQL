...
- 서브쿼리의 사용방법은 () 안에 명시, 서브쿼리절의 리턴행이 1줄 이하여야 한다.
- 서브쿼리 절에는 비교할 대상이 하나 반드시 들어가야한다.
...
-- ALL: 값을 서브쿼리에 의해 리턴된 값과 모두 비교해서 모두 만족해야한다.
SELECT *
FROM employees
...
/*
- 스칼라 서브쿼리가 조인보다 좋은 경우: 함수처럼 한 레코드당 정확히 하나의 값만을 리턴할 때

- 조인이 스칼라 서브쿼리보다 좋은 경우: 조회할 데이터가 대용량인 경우, 해당 데이터가 수정 또는 삭제 등이 빈번한 경우
*/
...
-- ROWNUM이 정렬되지 않는 상황 발생. 
-- 이유: ROWNUM이 먼저 붙고 정렬이 진행되기 때문. ORDER BY는 항상 마지막에 진행됨
-- 해결: 정렬이 미리 진행된 자료에 ROWNUM을 붙여서 다시 조회
SELECT ROWNUM AS rn, employee_id, first_name, salary
FROM employees
ORDER BY salary DESC;

-- ROWNUM을 붙이고 나서 범위를 지정해서 조회하려고 하는데, 범위 지정도 불가능하고 지목할 수 없는 문제 발생
-- 이유: WHERE절부터 실행하고 나서 ROWNUM이 SELECT되기 때문
-- 해결: ROWNUM까지 붙여놓고 다시 한 번 자료를 SELECT해서 범위를 지정
